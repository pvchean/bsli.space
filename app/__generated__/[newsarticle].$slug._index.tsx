...
<Time
country={"US"}
dateTime={`${CMSData?.data?.data?.[0]?.["date_created"]}`}
dateStyle={"medium"}
className={`w-time`} />
</div>
</div>
...
<Image
src={`${CMSAssetURL}${CMSData?.data?.data?.[0]?.image?.id}`}
width={CMSData?.data?.data?.[0]?.image?.width}
height={CMSData?.data?.data?.[0]?.image?.height}
...
<p
className={`w-element c1b04qto c1tzvcmb`}>
{`${CMSData?.data?.data?.[0]?.text}`}
</p>
</div>
</Box>
</Box>
